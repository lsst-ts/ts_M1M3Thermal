...
void SavedSetpoints::load() {
    try {
        ...
        _glycol = doc["Setpoints"]["Glycol"].as<float>(NAN);
        _heaters = doc["Setpoints"]["Heaters"].as<float>(NAN);
...
bool SavedSetpoints::is_valid() { return (!std::isnan(_heaters) && !std::isnan(_glycol)); }
